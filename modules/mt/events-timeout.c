...
volatile int esq_mt_handled_timedout_events = 0;

void
esq_mt_timeoout_event_cb(void * current_session)	{
	int events;
	void * event_param;
	...
	if ((ESQ_EVENT_BIT_TIMEOUT == events))	{
		...
		esq_mt_handled_timedout_events ++;
...
int
esq_mt_single_shot_socket_pt_handle_timeout_event(int session_cnt, int timeout, int wait_usecond)	{
	void * session;
	int r;
	int i;
	int fd = -100;
	...
	esq_mt_handled_timedout_events = 0;

	for (i = 0; i < session_cnt; i ++)	{

		session = esq_session_new("mt-timeout-session");
		if (!session)	{
		...
		r = esq_session_set_socket_fd(session, fd, esq_mt_timeoout_event_cb
				, (ESQ_EVENT_BIT_TIMEOUT | ESQ_EVENT_BIT_READ | ESQ_EVENT_BIT_WRITE)
				, 0, timeout, ESQ_TST_PARAM
				);
		if (r)	{
	...
	if (esq_mt_handled_timedout_events == session_cnt)	{
		...
		printf("ERROR: %s() %d: %d event(s) reported, not %d\n",
				__func__, __LINE__,
				esq_mt_handled_timedout_events, session_cnt
				);
		return -1;
	}
}
